...
import type { RolesQueryState } from '@/features/roles/types'
...
export function RoleToolbar({
  ...
  query: RolesQueryState
  searchValue: string
  searchPlaceholder: string
  onSearchValueChange: (value: string) => void
  onStatusChange: (status: RolesQueryState['status']) => void
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-3.5 sm:px-5">
        ...
        <div className="relative">
          <select
            value={query.status}
            onChange={(e) =>
              onStatusChange(e.target.value as RolesQueryState['status'])
            }
