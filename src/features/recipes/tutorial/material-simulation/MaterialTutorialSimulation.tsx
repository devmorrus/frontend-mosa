...
function TutorialDeviationPanel({ deviation, reason, onReasonChange, onRequest, onApprove, onReject }: { deviation: { status: string; reason: string; reviewNotes: string | null } | undefined; reason: string; onReasonChange: (value: string) => void; onRequest: () => void; onApprove: () => void; onReject: () => void }) {
  ...
  if (deviation?.status === 'APPROVED') return <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><b>Simulasi approved.</b> Actual ini menjadi dasar pengurangan stock pada production nyata, tetapi tutorial tidak membuat transaksi.</div>
