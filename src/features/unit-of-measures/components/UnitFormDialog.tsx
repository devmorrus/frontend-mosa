...
function HeaderIcon({ mode }: { mode: 'create' | 'edit' }) {
  return (
    ...
    >
      {mode === 'create' ? <Ruler size={20} /> : <PencilLine size={20} />}
    </div>
  )
}
...
function UnitPreview({ values }: { values: UnitOfMeasureFormValues }) {
  const displayName = values.name.trim() || 'Nama unit'
  const [from, to] = getAvatarGradient(displayName)
  const symbolLabel = values.symbol.trim() || 'Tanpa simbol'
...
export function UnitFormDialog({
  ...
  submitting: boolean
  onValuesChange: (updater: (prev: UnitOfMeasureFormValues) => UnitOfMeasureFormValues) => void
  onOpenChange: (open: boolean) => void
  onSubmit: () => void
}) {
  const title = mode === 'create' ? 'Tambah Unit' : 'Edit Unit'
  const submitLabel = mode === 'create' ? 'Simpan Unit' : 'Perbarui Unit'
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <DialogDescription className="mt-0.5 text-[13px] text-slate-500">
                  {mode === 'create'
                    ? 'Isi data unit baru agar siap dipakai lintas modul.'
                    : 'Perbarui informasi unit sesuai standar operasional terkini.'}
                </DialogDescription>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <FieldHelper>Kode unik untuk identifikasi unit</FieldHelper>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <FieldLabel required>Nama Unit</FieldLabel>
                  <IconInput icon={<Ruler size={15} />}>
                    <Input
                      value={values.name}
                      onChange={(event) =>
                  ...
                  <FieldHelper>Nama satuan untuk digunakan lintas modul</FieldHelper>
                </div>
                ...
                <div>
                  ...
                  <IconInput icon={<Ruler size={15} />}>
                    <Input
                      value={values.symbol}
                      onChange={(event) =>
                        onValuesChange((prev) => ({ ...prev, symbol: event.target.value }))
                      }
                      placeholder="kg"
                      className="h-12"
                    />
                  </IconInput>
                  <MasterDataFormFieldError message={getFieldError(errors, 'symbol')} />
                  <FieldHelper>Singkatan tampilan satuan bila dipakai</FieldHelper>
                </div>
              </div>
            </div>
          )}
