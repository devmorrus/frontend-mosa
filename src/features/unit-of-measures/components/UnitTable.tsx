import { Hash, Pencil, RotateCcw, Ruler } from 'lucide-react'
...
import type { UnitOfMeasureListItem } from '@/features/unit-of-measures/types'
...
function getSymbolLabel(symbol: string | null) {
  return symbol?.trim() || 'Tanpa simbol'
}

function UnitAvatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' }) {
...
function ActionMenu({
  ...
  onEdit: (item: UnitOfMeasureListItem) => void
  onToggleStatus: (item: UnitOfMeasureListItem) => void
}) {
  return (
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
      <div className="flex gap-3">
        <UnitAvatar name={item.name} size="sm" />
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex flex-col gap-1.5 border-t border-slate-100 pt-3">
            <SpecLine
              icon={<Ruler size={12} />}
              label="Symbol"
              value={getSymbolLabel(item.symbol)}
            />
          </div>
    ...
    <Card className="overflow-hidden rounded-[20px] border border-slate-200/75 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
      <CardHeader className="border-b border-slate-100 bg-white px-5 py-4 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <h2 className="font-display text-base font-semibold text-slate-900">Unit List</h2>
            <p className="mt-0.5 truncate text-xs text-slate-500">
              <span className="font-semibold text-slate-900">{pagination.totalItems}</span> unit terdaftar
