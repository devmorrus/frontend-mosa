...
export function MaterialConsumptionPanel({ step, uom, disabled, onValidateLot, onSubmit }: { step: OperatorProductionCurrentStep; uom: string; disabled: boolean; onValidateLot: (value: string) => Promise<ValidatedMaterialLot>; onSubmit: (lots: ValidatedMaterialLot[], reason: string | null) => void }) {
  ...
  return <div className="mt-6 space-y-4 rounded-2xl border border-ink/10 p-5"><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><Input data-tour="consume-lot-select" value={scanValue} onChange={(event) => setScanValue(event.target.value)} placeholder="Scan QR atau masukkan ID LOT" disabled={disabled} /><Button type="button" onClick={() => void scan()} disabled={disabled || !scanValue.trim()}><ScanLine size={16} />Validasi LOT</Button></div>{scanError ? <p className="text-sm text-red-700">{scanError}</p> : null}
    ...
    <div data-tour="consume-tolerance-hint" className="rounded-xl bg-sand/45 p-4 text-sm"><div className="grid gap-2 sm:grid-cols-3"><span>Target: <b>{step.targetQuantity} {uom}</b></span><span>Total Actual: <b>{total} {uom}</b></span><span>Variance: <b>{tolerance.variance} {uom}</b></span></div>{tolerance.lowerLimit !== null || tolerance.upperLimit !== null ? <p className="mt-2">Allowed range: {tolerance.lowerLimit ?? '-'} - {tolerance.upperLimit ?? '-'} {uom}</p> : null}</div>
