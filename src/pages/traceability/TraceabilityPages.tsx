...
function statusTone(value: string | number | null | undefined, kind: StatusKind) {
  ...
  if (status.includes('pass') || status.includes('available') || status.includes('complete')) return 'border-emerald-200 bg-emerald-50 text-emerald-700'
...
export function TraceabilitySearchPage() {
  ...
  return <div className="space-y-6 pb-10">
    <section className="relative overflow-hidden rounded-[30px] bg-ink px-6 py-8 text-paper shadow-[0_24px_80px_rgba(18,48,46,0.16)] sm:px-8 lg:px-10">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_88%_12%,rgba(232,163,61,0.30),transparent_25%),radial-gradient(circle_at_10%_100%,rgba(88,156,142,0.22),transparent_40%)]" />
    ...
    <Card className="border-white/70 bg-white shadow-sm"><CardContent className="p-5 sm:p-7">
      <div className="flex flex-wrap gap-2"><SearchTypeButton active={searchType === 'finished-goods'} onClick={() => { setSearchType('finished-goods'); setResult(null); setForwardResult(null); setError(null) }} icon={<PackageCheck size={16} />} label="Finished Goods LOT" /><SearchTypeButton active={searchType === 'raw-material'} onClick={() => { setSearchType('raw-material'); setResult(null); setForwardResult(null); setError(null) }} icon={<Boxes size={16} />} label="Raw Material LOT" /></div>
      <form className="mt-5 flex flex-col gap-3 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void search() }}><Input aria-label="Masukkan LOT Number" value={value} onChange={(event) => setValue(event.target.value)} placeholder={searchType === 'finished-goods' ? 'Masukkan LOT Number, contoh FG-20260902-00001' : 'Masukkan Raw Material LOT'} className="h-12 flex-1 rounded-2xl" /><Button type="submit" className="h-12" disabled={isSearching}>{isSearching ? 'Mencari...' : <><Search size={16} />Cari LOT</>}</Button>{searchType === 'finished-goods' ? <Button type="button" variant="secondary" className="h-12" onClick={() => void openCamera()}><QrCode size={16} />Scan QR</Button> : null}</form>
...
export function FinishedGoodsTraceabilityPage() {
  ...
  return <div className="space-y-6 pb-10"><div><div className="text-xs font-semibold uppercase tracking-[0.16em] text-signal">Backward traceability</div><h1 className="mt-2 font-display text-3xl font-semibold text-ink">Finished Goods genealogy</h1></div><GenealogyResult data={data} /></div>
}
