...
export function ProductionDeviationReviewPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6"><Button asChild variant="secondary"><Link to="/production/deviations"><ArrowLeft size={16} />Kembali ke Deviation Queue</Link></Button><section className="rounded-[30px] bg-ink px-6 py-7 text-paper sm:px-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-signal">Deviation Review</div><h1 className="mt-3 font-display text-3xl font-semibold">{detail.productionOrder.productionOrderNumber}</h1><p className="mt-2 text-sm text-paper/65">{detail.productionOrder.productCode} - {detail.productionOrder.productName}</p></div><DeviationStatusBadge status={detail.decision.status} /></div></section>
    <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Production Information</CardTitle></CardHeader><CardContent className="grid gap-4 text-sm sm:grid-cols-2"><Info label="Production Order" value={detail.productionOrder.productionOrderNumber} /><Info label="Product" value={`${detail.productionOrder.productCode} - ${detail.productionOrder.productName}`} /><Info label="Recipe" value={`${detail.productionOrder.recipeName} V${detail.productionOrder.recipeVersionNumber}`} /><Info label="Warehouse" value={`${detail.productionOrder.warehouseCode} - ${detail.productionOrder.warehouseName}`} /><Info label="Target Output" value={`${detail.productionOrder.targetOutput} ${detail.productionOrder.unitOfMeasureSymbol ?? detail.productionOrder.unitOfMeasureCode}`} /><Info label="Step" value={`${detail.step.sequence}. ${detail.step.stepName}`} /></CardContent></Card><Card><CardHeader><CardTitle>Material Requirement</CardTitle></CardHeader><CardContent className="grid gap-4 text-sm sm:grid-cols-2"><Info label="Material" value={`${detail.step.rawMaterialCode ?? '-'} - ${detail.step.rawMaterialName ?? '-'}`} /><Info label="Target" value={`${detail.material.targetQuantity} ${uom}`} /><Info label="Tolerance" value={tolerance(detail)} /><Info label="Allowed Range" value={detail.material.lowerLimit !== null && detail.material.upperLimit !== null ? `${detail.material.lowerLimit} - ${detail.material.upperLimit} ${uom}` : '-'} /><div className="sm:col-span-2 rounded-2xl bg-sand/35 p-4 text-slate-700">{detail.step.instruction ?? 'Tidak ada instruksi.'}</div></CardContent></Card></div>
    <Card><CardHeader><CardTitle>Actual Usage</CardTitle><CardDescription>Total actual {detail.material.actualQuantity} {uom}; variance {detail.material.varianceQuantity} {uom}.</CardDescription></CardHeader><CardContent className="px-0 pb-0"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>{['LOT', 'Supplier LOT', 'Warehouse', 'Expiry', 'Available', 'Actual'].map((label) => <th key={label} className="px-5 py-3">{label}</th>)}</tr></thead><tbody>{detail.lots.map((lot) => <tr key={lot.materialConsumptionId} className="border-t border-slate-100"><td className="px-5 py-4 font-semibold">{lot.internalLotNumber}</td><td className="px-5 py-4">{lot.supplierLot ?? '-'}</td><td className="px-5 py-4">{lot.warehouseCode}</td><td className="px-5 py-4">{date(lot.expiryDate)}</td><td className="px-5 py-4">{lot.availableStock} {uom}</td><td className="px-5 py-4">{lot.proposedActualQuantity} {uom}</td></tr>)}</tbody></table></div></CardContent></Card>
    <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Operator Request</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><Info label="Operator" value={detail.request.operator} /><Info label="Request Time" value={date(detail.request.requestedAtUtc)} /><div className="rounded-2xl bg-sand/35 p-4 leading-6 text-slate-700">{detail.request.reason}</div></CardContent></Card><Card><CardHeader><CardTitle>Decision</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><Info label="Status" value={pending ? 'Menunggu keputusan supervisor' : ''} badge={<DeviationStatusBadge status={detail.decision.status} />} /><Info label="Reviewed By" value={detail.decision.reviewedBy ?? '-'} /><Info label="Reviewed At" value={date(detail.decision.reviewedAtUtc)} />{detail.decision.reviewNotes ? <div className="rounded-2xl bg-sand/35 p-4 leading-6 text-slate-700">{detail.decision.reviewNotes}</div> : null}{pending && canReview ? <div className="flex flex-wrap gap-3 pt-2"><Button onClick={() => { setDialog('approve'); setNotes(''); setSubmitError(null) }}><ShieldCheck size={16} />Approve</Button><Button variant="secondary" onClick={() => { setDialog('reject'); setNotes(''); setSubmitError(null) }}><ShieldX size={16} />Reject</Button></div> : null}</CardContent></Card></div>
...
function Info({ label, value, badge }: { label: string; value: string; badge?: React.ReactNode }) { return <div><div className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</div><div className="mt-1 font-semibold text-ink">{badge ?? value}</div></div> }
