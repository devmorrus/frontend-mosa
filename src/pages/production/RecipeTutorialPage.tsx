...
export function RecipeTutorialPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    <section className="rounded-[30px] bg-ink px-6 py-7 text-paper sm:px-8"><div className="flex items-start justify-between gap-4"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-signal">Guided Recipe Tutorial</div><h1 className="mt-3 font-display text-3xl font-semibold">{preview.recipeName}</h1><p className="mt-2 text-sm text-paper/65">Version V{preview.versionNumber}</p></div>{isStarted ? <Button type="button" variant="ghost" onClick={() => setExitOpen(true)}><X size={16} />Keluar</Button> : null}</div></section>
    ...
    {isCompleted ? <Card><CardHeader><CardTitle>Produksi selesai</CardTitle><CardDescription>{preview.recipeName} V{preview.versionNumber} telah dipelajari dalam SIMULASI TUTORIAL.</CardDescription></CardHeader><CardContent className="space-y-5"><div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">{[RecipeStepType.Material, RecipeStepType.Process, RecipeStepType.Timer, RecipeStepType.Check].map((type) => <div key={type} className="rounded-2xl bg-sand/35 p-4"><div className="text-slate-500">{type === 1 ? 'Material' : type === 2 ? 'Process' : type === 3 ? 'Timer' : 'Check'}</div><div className="mt-1 text-2xl font-semibold">{preview.steps.filter((step) => step.stepType === type).length}</div></div>)}</div><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-ink/10 p-4 text-sm"><b>Target / Actual</b><p className="mt-2">{preview.previewTargetOutput} / {progress?.finalSimulation.actualOutput} {preview.outputUnitOfMeasure.symbol ?? preview.outputUnitOfMeasure.code}</p><p className="mt-1">Yield: {Number(((Number(progress?.finalSimulation.actualOutput.replace(',', '.')) / preview.previewTargetOutput) * 100).toFixed(2))}%</p></div><div className="rounded-2xl border border-ink/10 p-4 text-sm"><b>Finished Goods LOT</b><p className="mt-2">{progress?.finalSimulation.finishedGoodsLotNumber}</p><p className="mt-1">QC: {progress?.finalSimulation.qcResult} · Inventory: {progress?.finalSimulation.qcResult === 'PASS' ? 'AVAILABLE' : progress?.finalSimulation.qcResult === 'HOLD' ? 'BLOCKED' : 'NON-AVAILABLE'}</p></div></div><p className="rounded-2xl bg-sand/35 p-4 text-sm text-slate-600">Material, timer, FG LOT, dan QC di atas hanya simulasi lokal. Tidak ada transaksi produksi, inventory, atau QC nyata.</p><div className="flex flex-wrap gap-3"><Button type="button" onClick={() => restart(userId)}><RotateCcw size={16} />Ulangi Tutorial</Button><Button asChild variant="secondary"><Link to={`/production/recipes/${recipeId}/versions/${versionId}`}>Kembali ke Recipe</Link></Button><Button variant="secondary" onClick={() => { close(); navigate(`/production/recipes/${recipeId}`) }}>Tutup</Button></div></CardContent></Card> : null}
    <Dialog open={exitOpen} onOpenChange={setExitOpen}><DialogContent><DialogHeader><DialogTitle>Tutorial belum selesai</DialogTitle><DialogDescription>Progres tersimpan di perangkat ini dan dapat dilanjutkan nanti.</DialogDescription></DialogHeader><DialogFooter><Button variant="secondary" onClick={() => setExitOpen(false)}>Lanjutkan Tutorial</Button><Button onClick={() => { close(); navigate(`/production/recipes/${recipeId}`) }}>Keluar Tutorial</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
