...
export function RecipeTutorialPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    {isCompleted ? <Card><CardHeader><CardTitle>Tutorial resep selesai</CardTitle><CardDescription>{preview.recipeName} V{preview.versionNumber} telah dipelajari.</CardDescription></CardHeader><CardContent className="space-y-5"><div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">{[RecipeStepType.Material, RecipeStepType.Process, RecipeStepType.Timer, RecipeStepType.Check].map((type) => <div key={type} className="rounded-2xl bg-sand/35 p-4"><div className="text-slate-500">{type === 1 ? 'Material' : type === 2 ? 'Process' : type === 3 ? 'Timer' : 'Check'}</div><div className="mt-1 text-2xl font-semibold">{preview.steps.filter((step) => step.stepType === type).length}</div></div>)}</div><div className="flex flex-wrap gap-3"><Button type="button" onClick={() => restart(userId)}><RotateCcw size={16} />Ulangi Tutorial</Button><Button asChild variant="secondary"><Link to={`/production/recipes/${recipeId}/versions/${versionId}`}>Lihat Recipe</Link></Button></div></CardContent></Card> : null}
