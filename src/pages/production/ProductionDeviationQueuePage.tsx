...
export function ProductionDeviationQueuePage() {
  ...
  useEffect(() => {
    ...
    void productionDeviationsApi.list(query).then((result) => { if (active) { setItems(result.items); setPagination(result.pagination) } }).catch((caught: ApiError) => active && setError(caught.message)).finally(() => active && (setLoading(false), setRefreshing(false)))
    return () => { active = false }
